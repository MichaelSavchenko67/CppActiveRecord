...
Table Sqlite3Connection::table_data(const std::string& table_name) {
  ...
  for(auto& row: rows) {
    ...
    {
      const std::string error = "Unknown type: " + type_name;
...
sqlite3_stmt* Sqlite3Connection::prepare(
  ...
  if(prepare_result != SQLITE_OK)
  {
    ...
    throw ActiveRecordException(errMsg.c_str(), __FILE__, __LINE__);
  }
  ...
  for(auto& parameter: parameters) {
    switch(parameter.which()) {
      case Type::integer: {
        ...
        sqlite3_bind_int64(ppStmt, i + 1, value);
        break;
      }
...
bool Sqlite3Connection::sqlite_initialize(std::string& database_path_name) {
  ...
  if(nResult != 0)
  {
    const std::string error = "Can't open database '" + database_path_name + "'" + std::string(sqlite3_errmsg(db_));
    sqlite3_close(db_);
    throw ActiveRecordException(error.c_str(), __FILE__, __LINE__);
  }
  return true;
